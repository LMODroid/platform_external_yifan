...
public class BitmapUtils {
    ...
    public static Bitmap getBitmap(ImageProxy image) {
        ...
        ByteBuffer nv21Buffer =
                yuv420ThreePlanesToNV21(
                        Objects.requireNonNull(image.getImage()).getPlanes(),
                        image.getWidth(),
                        image.getHeight());
     ...
     *
     * <p>YUV_420_888 is a generic format that can describe any YUV image where U and V are
     * subsampled by a factor of 2 in both dimensions. {@link Image#getPlanes} returns an array with
     * the Y, U and V planes. The Y plane is guaranteed not to be interleaved, so we can just copy
     * its values into the first part of the NV21 array. The U and V planes may already have the
     * representation in the NV21 format. This happens if the planes share the same buffer, the V
     * buffer is one position before the U buffer and the planes have a pixelStride of 2. If this is
     * case, we can just copy them to the NV21 array.
     */
