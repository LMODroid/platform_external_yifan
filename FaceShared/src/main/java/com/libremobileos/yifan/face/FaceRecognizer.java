...
/**
 * Task-specific API for detecting &amp; recognizing faces in an image. Uses {@link FaceFinder} to
 * detect and scan faces, {@link FaceStorageBackend} to store and retrieve the saved faces and
 * returns the optimal result.<br>
 * Refrain from using this class for registering faces into the recognition system, {@link
 * FaceFinder} does not perform post processing and is as such better suited.
 */
public class FaceRecognizer {
    ...
    private FaceRecognizer(
            Context ctx,
            FaceStorageBackend storage,
            float minConfidence,
            int inputWidth,
            int inputHeight,
            int sensorOrientation,
            float maxDistance,
            int minMatchingModels,
            float minModelRatio,
     ...
     * @param minConfidence Minimum confidence to track a detection, must be higher than 0.0f and
     *     smaller than 1.0f
     ...
     * @param maxDistance Maximum distance (difference, not 3D distance) to a saved face to count as
     *     recognized. Must be higher than 0.0f and smaller than 1.0f
     * @param minMatchingModels Minimum count of matching models for one face to count as
     *     recognized. If undesired, set to 1
     ...
     * @see #create(Context, FaceStorageBackend, float, int, int, int, float, float, boolean,
     *     boolean, int)
     ...
     * @param maxDistance Maximum distance (difference, not 3D distance) to a saved face to count as
     *     recognized. Must be higher than 0.0f and smaller than 1.0f
     * @param minModelRatio Minimum count of matching models for one face to count as recognized.
     *     Must be higher or equal to 0.0f and smaller or equal to 1.0f. If undesired, set to 0f
     * @param hwAcceleration Enable hardware acceleration (NNAPI/GPU)
     * @param enhancedHwAcceleration if hwAcceleration is enabled, use NNAPI instead of GPU. if not,
     *     this toggles XNNPACK
     * @param numThreads How many threads to use, if running on CPU or with XNNPACK
     * @return {@link FaceRecognizer} instance.
     * @see #create(Context, FaceStorageBackend, float, int, int, int, float, int, boolean, boolean,
     *     int)
     * @see #create(Context, FaceStorageBackend, float, int, int, int, float, float)
     * @see #create(Context, FaceStorageBackend, float, int, int, int, float, int)
     */
    ...
    /**
     * Create {@link FaceRecognizer} instance, with minimum matching model constraint and sensible
     * defaults regarding hardware acceleration (CPU, XNNPACK, 4 threads).
     *
     * @param ctx Android {@link Context} object, may be in background.
     * @param storage The {@link FaceStorageBackend} containing faces to be recognized.
     * @param minConfidence Minimum confidence to track a detection, must be higher than 0.0f and
    ...
    /**
     * Create {@link FaceRecognizer} instance, with matching model ratio constraint and sensible
     * defaults regarding hardware acceleration (CPU, XNNPACK, 4 threads).
     *
     * @param ctx Android {@link Context} object, may be in background.
     * @param storage The {@link FaceStorageBackend} containing faces to be recognized.
    ...
    /**
     * Stores a combination of {@link FaceScanner.Face} and {@link FaceDetector.Face}, for face
    ...
    public static class Face extends FaceScanner.Face {
        ...
        /* package-private */ Face(
                ...
                int modelCount,
                float modelRatio,
                float brightnessTest1,
                float brightnessTest2) {
        ...
        /* package-private */ Face(
                FaceScanner.Face original, Float confidence, int modelCount, float modelRatio) {
            this(
                    original.getId(),
                    original.getTitle(),
                    ...
                    modelCount,
                    modelRatio,
                    original.brightnessTest1,
                    original.brightnessTest2);
        }

        /* package-private */ Face(
                FaceDetector.Face raw,
                FaceScanner.Face original,
                int modelCount,
                float modelRatio) {
        ...
        /**
         * A score for how good the detection (NOT recognition, that's {@link #getDistance()}) is
         * relative to others.
         *
    ...
    public List<Face> recognize(Bitmap input) {
        ...
        final List<Pair<FaceDetector.Face, FaceScanner.Face>> faces =
                detector.process(
                        input,
        ...
        for (Pair<FaceDetector.Face, FaceScanner.Face> faceFacePair : faces) {
            FaceDetector.Face found =
                    faceFacePair.first; // The generic Face object indicating where a Face is
            FaceScanner.Face scanned =
                    faceFacePair.second; // The Face object with face-scanning data
            ...
            for (String savedName : savedFaces) {
                ...
                for (float[] data : rawData) {
                    ...
                    if (newDistance < maxDistance) {
                        matchingModels++;
                        if (finalDistance > newDistance) finalDistance = newDistance;
                    }
                }
                float modelRatio = (float) matchingModels / rawData.length;
                // If another known face had better similarity, don't save it
                if (minModelRatio > 0
                        ? minModelRatio < modelRatio
                        : matchingModels >= Math.min(rawData.length, minMatchingModels)
                                && finalDistance < scanned.getDistance()) {
