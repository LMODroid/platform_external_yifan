 ...
 * facial features. Most likely, specialized classes like {@link FaceRecognizer} or {@link
 * FaceFinder} fit your use case better.
 */
public class FaceScanner {
    ...
    /**
     * Processes Bitmaps to compatible format. This class supports 2 modes of operation:<br>
     * 1. Preprocess perfectly cropped {@link Bitmap} to AI-compatible format, using the static
     * method {@link #process(Bitmap, int)}<br>
     * 2. Crop one large {@link Bitmap} to multiple {@link InputImage}s using bounds inside {@link
     * RectF} objects, with {@link #InputImageProcessor(Bitmap, int)} and {@link #process(RectF)}.
     * This allows processing multiple faces on one {@link Bitmap}, for usage with {@link
     * FaceDetector} and similar classes.
     *
     * @see InputImage
     */
    public static class InputImageProcessor {
        ...
        /**
         * If the class gets instantiated, we enter a special mode of operation for detecting
         * multiple faces on one large {@link Bitmap}.
         *
        ...
        /**
         * In normal mode of operation, we take a perfectly cropped {@link Bitmap} containing one
         * face and process it.
         *
        ...
        /**
         * In normal mode of operation, we take a perfectly cropped {@link Bitmap} containing one
         * face and process it. This utility method uses sensorOrientation that was passed in the
         * constructor and calls {@link #process(Bitmap, int)}
         *
         * @param input Bitmap to process.
         * @return Converted {@link InputImage}
         ...
         * {@link Bitmap} (in multiple passes).
         *
        ...
        public InputImage process(RectF inputBB) {
            ...
            if (faceBB.left < 0
                    ...
                    || faceBB.bottom < 0
                    || faceBB.right < 0
                    || (faceBB.left + faceBB.width()) > portraitBmp.getWidth()
    ...
    private float[] brightnessTest(int color) {
        Bitmap b =
                Bitmap.createBitmap(
                        FaceScanner.TF_OD_API_INPUT_SIZE,
                        FaceScanner.TF_OD_API_INPUT_SIZE,
                        Bitmap.Config.ARGB_8888);
    ...
    public static class Face {
        ...
        /* package-private */ Face(
                ...
                final Bitmap crop,
                final float[] extra,
                final float brightnessTest1,
                final float brightnessTest2) {
            this.id = id;
            this.title = title;
        ...
        /**
         * A score for how good the recognition is relative to others. Do not confuse with 3D
         * distance, this is entirely about recognition.
         *
        ...
        /**
         * Test if the face has already been recognized (if {@link #addRecognitionData(String,
         * float)} has been called)
         *
        ...
        public int getBrightnessHint() {
            return (brightnessTest1 < 0.5f || brightnessTest2 < 0.4f)
                    ? -1
                    : // really bad light
                    (brightnessTest1 + brightnessTest2 < 2.2f
                            ? 0 // suboptimal
                            : 1); // optimal
        }

        /**
         * Static method to compare two {@link Face}s. Usually, one of the instance methods is used
         * though.
         *
     ...
     * @param enhancedHwAcceleration if hwAcceleration is enabled, use NNAPI instead of GPU. if not,
     *     this toggles XNNPACK
     * @param numThreads How many threads to use, if running on CPU or with XNNPACK
     * @return {@link FaceScanner} instance.
     * @see #create(Context)
     */
    public static FaceScanner create(
            Context context,
            boolean hwAcceleration,
            boolean enhancedHwAcceleration,
            int numThreads) {
        AssetManager assetmanager = null;
        if (context != null) assetmanager = context.getAssets();
        return new FaceScanner(assetmanager, hwAcceleration, enhancedHwAcceleration, numThreads);
    }

    /**
     * Create {@link FaceScanner} instance with sensible defaults regarding hardware acceleration
    ...
    private FaceScanner(
        ...
        this.brightnessTest =
                new float[][] {brightnessTest(Color.WHITE), brightnessTest(Color.BLACK)};
    }

    private SimilarityClassifier getClassifier() throws IOException {
        if (classifier == null) {
            classifier =
                    SimilarityClassifier.create(
                            am,
     ...
     * @param allowPostprocessing Allow postprocessing to improve detection quality. Undesirable
    ...
    public Face detectFace(InputImage input, boolean allowPostprocessing) {
        try {
            List<SimilarityClassifier.Recognition> results =
                    getClassifier().recognizeImage(input.getProcessedImage());
            ...
            Face f =
                    new Face(
                            ...
                            result.getDistance(),
                            null,
                            input.getUserDisplayableImage(),
                            e,
                            Face.compare(e, brightnessTest[0]),
                            Face.compare(e, brightnessTest[1]));
            if (f.getBrightnessHint() == 0
                    && allowPostprocessing /* try to improve light situation with postprocessing if its bad but not terrible */) {
                Face f2 =
                        detectFace(
                                new InputImage(
                                        doBrightnessPostProc(input.getProcessedImage()),
                                        doBrightnessPostProc(input.getUserDisplayableImage())),
                                false);
                if (f2 == null) // Earlier logs will have printed the cause.
                return null;
                if (f2.getBrightnessHint() == 1) return f2; // Return if it helped.
            }
            return f;
        } catch (IOException e) {
    ...
    private Bitmap doBrightnessPostProc(Bitmap input) {
        // 30, which has been obtained using manual testing, gives the best balance between
        // brightness and trashing facial features
