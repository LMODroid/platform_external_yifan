...
/**
 * Detect multiple faces in one large {@link Bitmap} and returns {@link Face} objects. Requires
 * preprocessed {@link InputImage} objects from {@link InputImageProcessor}.
 */
public class FaceDetector {
    ...
    public static class InputImageProcessor {
        ...
        public InputImage process(Bitmap input) {
            Bitmap croppedBitmap =
                    Bitmap.createBitmap(
                            TF_FD_API_INPUT_SIZE, TF_FD_API_INPUT_SIZE, Bitmap.Config.ARGB_8888);
    ...
    public static class Face {
        ...
        /* package-private */ Face(final String id, final Float confidence, final RectF location) {
            this.id = id;
    ...
    public static FaceDetector create(
        ...
        return new FaceDetector(
                assetmanager, minConfidence, hwAcceleration, enhancedHwAcceleration, numThreads);
    }

    /**
     * Create {@link FaceDetector} instance with sensible defaults regarding hardware acceleration
        ...
        try {
            ...
            for (final SimilarityClassifier.Recognition result : results) {
                ...
                if (location != null && result.getDistance() >= minConfidence) {
                    ...
                    mappedRecognitions.add(
                            new Face(result.getId(), result.getDistance(), location));
                }
            }
