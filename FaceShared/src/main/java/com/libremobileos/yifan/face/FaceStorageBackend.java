...
/**
 * Store Faces on disk (or in memory, or anywhere else, really). This abstract class already
 * performs error checking, caching and data type conversion for both users and implementations.
 * Creating a new implementation only requires any key-value store that can store Base64-encoded
 * strings. An implementation is required to use this class.
 *
...
public abstract class FaceStorageBackend {
    ...
    public Set<String> getNames() {
        ...
        return (cachedNames =
                getNamesInternal().stream()
                        .map(v -> new String(decoder.decode(v), StandardCharsets.UTF_8))
     ...
     * @param replace Allow replacing an already registered face (based on name). If false and it's
     *     still attempted, the method returns false and does nothing.
    ...
    /**
     * Register/store new face. Calls {@link #register(String, float[][], boolean)} and does not
     * allow replacements.
     *
     * @param rawname Name of the face, needs to be unique.
     * @param alldata Face detection model data to store.
    ...
    /**
     * Store 1D face model by converting it to 2D and then calling {@link #register(String,
     * float[][])}.<br>
     * Implementation looks like this: <code>return register(rawname, new float[][] { alldata })
     * </code>).<br>
     *
     * @param rawname Name of the face, needs to be unique.
    ...
    public float[][] get(String name) {
        ...
        if (f != null) return f;
        f =
                FaceDataEncoder.decode(
                        getInternal(encoder.encodeToString(name.getBytes(StandardCharsets.UTF_8))));
